      ...
      module AuthProvider
        class ConfigCommand < Kontena::Command
          ...
          PRESETS = {
            ...
            kontena: {
              'authorize_endpoint'         => 'https://cloud-beta.kontena.io/login/oauth/authorize',
              'code_requires_basic_auth'   => false,
              ...
              'userinfo_username_jsonpath' => '$..username',
              'userinfo_email_jsonpath'    => '$..email',
              'userinfo_user_id_jsonpath'  => '$..id',
              'userinfo_scope'             => 'user'
            }
          ...
          option ['--auth-preset'], '[NAME]', "Use preset provider configuration. Providers: #{PRESETS.keys.join(",")}"
          ...
          def execute
            ...
            self.auth_preset ||= 'kontena' if kontena_auth?
            ...
            elsif self.auth_preset
              if PRESETS.has_key?(self.auth_preset.to_sym)
                settings = PRESETS[self.auth_preset.to_sym].merge(config_from_params)
              else
            ...
            if !is_kontena?(settings) && (settings['client_id'].nil? || settings['client_secret'].nil?)
              ...
              exit 1
            elsif is_kontena?(settings) && settings['client_id'].nil?
              if kontena_auth?
                ENV["DEBUG"] && puts("Creating Kontena Cloud OAuth2 Application")
                client = Kontena::Client.new(kontena_account.url, kontena_account.token)
                uri = URI.parse(current_master.url)
                uri.path = '/cb'
                puts uri.to_s
                ...
                ENV["DEBUG"] && puts("App creation response: #{response.inspect}")
                if response && response.kind_of?(Hash)
                  settings['client_id']     = response['client_id']
                  settings['client_secret'] = response['client_secret']
                end
              else
                puts "You must be authenticated to Kontena Cloud or supply preconfigured client_id + client_secret"
                exit 1
              end
            end
            ...
            if response && response.kind_of?(Hash)
              if response.has_key?('error')
                ...
                if current_master.username == 'admin'
                  puts "Authentication provider settings updated."
                else
